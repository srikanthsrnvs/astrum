        ...
        if type == 'conv_1d':
            if input_dims == None:
                layer = keras.layers.Conv1D(
                    filters, kernel, activation=activation)
            else:
                layer = keras.layers.Conv1D(
                    filters, kernel, activation=activation, input_shape=input_dims)
        elif type == 'conv_2d':
            if input_dims == None:
    ...
    def build_random_arch(self, identifier, input_shape, output_size, output_activation, min_layer_size=2, max_layer_size=1000, plot=False):
        ...
        for layer_num in range(1, num_hidden_layers+1):
            ...
            if len(layer_dict.keys()) > 0 and layer_dict[layer_num-1]["type"] != "dense":
                ...
                layer = self._build_post_conv_layer(
                    layer_type, pool_size=pool_size)
            else:
            ...
            if layer_type != 'dense':
                if len(layer_dict.keys()) == 0:
                    layer = self._build_conv_layer(
                        layer_type, layer_activation, input_dims=input_shape)
                else:
                    ...
                    if len(layer_dict.keys()) == 0:
                        ...
                        if layer_type == 'conv_1d' or layer_type == 'conv_2d' or layer_type == 'conv_3d':
                            layer_list.append(keras.layers.Reshape(
                                (28, 28, 1), input_shape=input_shape))
                            layer = self._build_conv_layer(
                            ...
                            layer = keras.layers.Dense(
                                layer_size, activation=layer_activation)
            ...
            layer_dict[layer_num] = {
                'size': layer_size, 'activation': layer_activation, 'type': layer_type}
            layer_list.append(layer)

        layer_dict[num_hidden_layers+1] = {'size': output_size,
                                           'activation': output_activation, 'type': 'Output'}

        layer_list.append(keras.layers.Dense(
        ...
        if plot:
            keras.utils.plot_model(model, to_file='{}.png'.format(
                identifier), show_shapes=True)
